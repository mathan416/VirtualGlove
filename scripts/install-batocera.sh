...
# File: scripts/install-batocera.sh
# Purpose: Download and verify the release, then install batocera integration.
# Author: Iain Bennett
...
p = argparse.ArgumentParser(description="Install VirtualGlove on batocera.")
...
try:
    ...
    if a.check:
        source = pathlib.Path("/userdata/system/virtualglove/scripts/setup-machine.py")
        if not source.is_file():
            raise ValueError("VirtualGlove is not installed at its standard location")
        cmd = ["python3", str(source), "batocera", "--check"]
        sys.exit(subprocess.call(cmd if os.geteuid() == 0 else ["sudo"] + cmd))
    root_console = "batocera" in ("recalbox", "batocera")
    ...
    if "batocera" != "uno-q" and a.hostname:
        raise ValueError("--hostname is available only with install-uno-q.sh")
    if "batocera" not in ("recalbox", "batocera") and (a.list_player1_devices or a.player1_device):
    ...
    temporary_root = ({"recalbox": "/recalbox/share/system",
                       "batocera": "/userdata/system"}.get("batocera"))
    with tempfile.TemporaryDirectory(prefix="virtualglove-download-",
        ...
        for line in (directory / "SHA256SUMS").read_text().splitlines():
            ...
            sums[name] = digest
        for name in ("install-package.py", "VirtualGlove-Batocera.zip"):
        ...
        cmd = ([] if os.geteuid() == 0 else ["sudo"]) + ["python3", str(directory / "install-package.py"), "batocera",
               "--archive", str(directory / "VirtualGlove-Batocera.zip"), "--version", tag]
        if a.peer:
            cmd += ["--peer", a.peer]
        if a.hostname:
