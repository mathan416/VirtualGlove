...
# File: scripts/install-retropie.sh
# Purpose: Download and verify the release, then install retropie integration.
# Author: Iain Bennett
...
p = argparse.ArgumentParser(description="Install VirtualGlove on retropie.")
...
try:
    ...
    root_console = "retropie" in ("recalbox", "batocera")
    ...
    if "retropie" != "uno-q" and a.hostname:
        raise ValueError("--hostname is available only with install-uno-q.sh")
    if "retropie" not in ("recalbox", "batocera") and (a.list_player1_devices or a.player1_device):
    ...
    temporary_root = ({"recalbox": "/recalbox/share/system",
                       "batocera": "/userdata/system"}.get("retropie"))
    with tempfile.TemporaryDirectory(prefix="virtualglove-download-",
        ...
        for line in (directory / "SHA256SUMS").read_text().splitlines():
            ...
            sums[name] = digest
        for name in ("install-package.py", "VirtualGlove-RetroPie.zip"):
        ...
        cmd = ([] if os.geteuid() == 0 else ["sudo"]) + ["python3", str(directory / "install-package.py"), "retropie",
               "--archive", str(directory / "VirtualGlove-RetroPie.zip"), "--version", tag]
        if a.peer:
            cmd += ["--peer", a.peer]
        if a.hostname:
