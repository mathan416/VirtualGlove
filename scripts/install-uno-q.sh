...
p = argparse.ArgumentParser(description="Install VirtualGlove on uno-q.")
...
try:
    ...
    root_console = "uno-q" in ("recalbox", "batocera")
    ...
    if "uno-q" != "uno-q" and a.hostname:
        raise ValueError("--hostname is available only with install-uno-q.sh")
    if "uno-q" not in ("recalbox", "batocera") and (a.list_player1_devices or a.player1_device):
    ...
    temporary_root = ({"recalbox": "/recalbox/share/system",
                       "batocera": "/userdata/system"}.get("uno-q"))
    with tempfile.TemporaryDirectory(prefix="virtualglove-download-",
        ...
        cmd = ([] if os.geteuid() == 0 else ["sudo"]) + ["python3", str(directory / "install-package.py"), "uno-q",
