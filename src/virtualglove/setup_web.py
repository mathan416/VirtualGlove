...
# File: src/virtualglove/setup_web.py
...
<section class=card id=connection-status-card aria-labelledby=connection-status-title><h2 id=connection-status-title>Connection status</h2><ul class=connection-indicators aria-label="Controller status checks" aria-live=polite>
<li id=status-app data-state=unknown><span class=check-dot aria-hidden=true></span><div><span class=check-label>Controller app</span><strong>Checking…</strong></div></li>
<li id=status-console data-state=unknown><span class=check-dot aria-hidden=true></span><div><span class=check-label>Console service</span><strong>Checking…</strong></div></li>
<li id=status-auth data-state=unknown><span class=check-dot aria-hidden=true></span><div><span class=check-label>Authenticated response</span><strong>Checking…</strong></div></li>
<li id=status-wifi data-state=unknown><span class=check-dot aria-hidden=true></span><div><span class=check-label>Networking</span><strong id=wifi-status>Checking…</strong></div></li>
...
<div class=setup-status-summary><p>Saved console: <strong id=status-destination>Loading…</strong></p><p>This Controller: <strong id=status-controller-identity>Loading…</strong></p><p>Active controller link: <strong id=status-active-destination>Not active</strong></p></div>
...
<section class=setup-phase id=setup-controllers aria-labelledby=setup-controllers-title>
<div class=setup-phase-heading><span class=setup-step>02 / CONTROLLERS</span><h2 id=setup-controllers-title>Choose your game controls</h2><p>After pairing, assign physical controllers and VirtualGlove to players. The console keeps its normal menu controls.</p></div>
{{ROUTER_CONTENT}}
</section>
...
<ol class=pair-progress aria-label="Pairing steps"><li id=pair-progress-1>Choose a method</li><li id=pair-progress-2>Confirm your Controller</li><li id=pair-progress-3>Pair with console</li></ol>
...
<label class=pair-choice><input type=radio name=pair-method value=code checked><span><strong>One-time code (recommended)</strong><br>Run the command for your console and enter the code it displays.</span></label>
<label class=pair-choice><input type=radio name=pair-method value=ssh><span><strong>SSH password</strong><br>Use your console username and password. SSH password login must be enabled.</span></label></fieldset>
...
<div id=pair-step-3 hidden><h3 id=pair-heading-3 tabindex=-1>3. Pair with your console</h3>
<fieldset id=pair-credentials disabled>
<div id=pair-code-fields><p>On the selected console, open a terminal and run:</p><pre><code id=pair-command>Select and save a console platform first.</code></pre><label>Console one-time code<input id=pair-code autocomplete=off placeholder=ABCDE-FGHIJ-23456-7ABCD></label><p>This code comes from that console, remains valid for five minutes, and is different from the six-digit Controller approval PIN.</p></div>
<div id=pair-ssh-fields hidden><div class=formgrid><label>Console username<input id=pair-user value=pi autocomplete=username></label><label>Console SSH password<input id=pair-password type=password autocomplete=off disabled></label></div><p id=pair-ssh-guidance>Your password is used for this pairing request and is not saved by the Controller.</p></div>
</fieldset><div class=controls><button id=pair-submit type=button disabled>Pair with console</button><button id=pair-review type=button class=secondary>Review Controller confirmation</button></div></div>
<div id=pair-pending hidden><h3 id=pair-pending-heading tabindex=-1>Pairing in progress</h3><p>Sending the pairing request to the console. Keep this page open while we wait for its response.</p><p>SSH pairing can take a few minutes. Pairing does not start controller output.</p></div>
<div id=pair-success hidden><img class=setup-pal-success src=/help-assets/gestures/v2/pixel-pal-success.png alt="Pixel Pal gives a thumbs-up"><h3 id=pair-success-heading tabindex=-1>Pairing complete</h3><p>The console receiver was restarted. The matrix resumes its normal display; when idle, it follows your attract setting. Open Dashboard to start controller output when you are ready. Pairing does not verify that a game received input.</p><a class=button href=/dashboard>Open Dashboard</a></div>
<p id=pair-notice role=status aria-live=polite aria-atomic=true tabindex=-1></p></div></div>
{{DOCTOR_CONTENT}}
</section>
<section id=controller-trust class=card aria-labelledby=trust-title><h2 id=trust-title>Trust this Controller</h2>
...
<details><summary>Install and trust the certificate</summary><ol class=compact-guidance><li>Before downloading, start Controller confirmation in <strong>Pair this Controller</strong> above and compare the browser certificate with the ID on the physical matrix.</li><li>Download the certificate only when those values match.</li><li>Install it as a trusted root on this device, then close and reopen the browser.</li></ol><p><strong>iPhone or iPad:</strong> install the downloaded profile, then open Settings → General → About → Certificate Trust Settings and enable full trust for VirtualGlove. <a href=https://support.apple.com/en-us/102390 rel="noreferrer noopener">Apple’s trust instructions</a> show the current steps.</p><p><strong>Mac:</strong> add it to your login keychain with Keychain Access, open the certificate, and set Secure Sockets Layer trust to Always Trust.</p><p><strong>Windows:</strong> import it for the current user into Trusted Root Certification Authorities.</p><p><strong>Android:</strong> install it as a CA certificate in the device’s security settings. Menu names vary by manufacturer.</p><p>Trust is local to this phone or computer. Repeat it on another device. Firefox may use a separate certificate store and require its own import. Removing or resetting the Controller authority requires trusting the replacement again.</p></details></section>
</section>
<section class=setup-phase id=setup-hand aria-labelledby=setup-hand-title>
<div class=setup-phase-heading><span class=setup-step>04 / HAND AND CAMERA</span><h2 id=setup-hand-title>Prepare your hand tracking</h2><p>Select a camera, choose a player, and adjust the movement area. Learn the gestures in <a href=/learn>Glove Academy</a>.</p></div>
<form id=camera-form><section id=camera-section class=card style="margin-bottom:14px"><h2>Camera</h2><fieldset id=camera-fields disabled style='border:0;padding:0;margin:0;min-width:0'><div class=formgrid><label>Camera<select id=camera name=camera><option value=auto>Automatic — choose the connected camera</option></select></label><label>Camera frame rate<select id=camera_fps name=camera_fps><option value=auto>Automatic — prefer 30 fps</option><option value=30>30 fps</option><option value=60>60 fps</option></select></label><label>Camera buffers<select id=camera_buffers name=camera_buffers><option value=1>1 buffer — lowest queue depth</option><option value=2>2 buffers — steadier camera delivery</option></select></label><label>Camera reader<select id=camera_backend name=camera_backend><option value=opencv>Recommended — OpenCV</option><option value=direct-v4l2>Engineering comparison — Direct V4L2</option></select></label><label>Exposure behaviour<select id=camera_exposure name=camera_exposure><option value=auto>Automatic — portable default</option><option value=low-latency>Automatic — fixed frame rate</option><option value=kiyo-low-latency>Automatic — Razer Kiyo Pro tested</option><option value=manual>Manual exposure and gain</option></select></label><label>Hand or glove (diagnostic label)<select id=glove_color name=glove_color><option value=none>Bare hand</option><option value=white>White glove</option><option value=black>Black glove</option></select></label></div><div id=camera-manual-settings class=formgrid hidden><label>Manual exposure<input id=camera_manual_exposure name=camera_manual_exposure type=number min=1 max=10000 step=1 inputmode=numeric></label><label>Manual gain<input id=camera_manual_gain name=camera_manual_gain type=number min=0 max=10000 step=1 inputmode=numeric></label></div>
<p class=setup-status-note id=camera-rate-status role=status>Actual camera behaviour appears while tracking is active.</p><ul class=compact-guidance><li>Automatic and OpenCV are the recommended starting point and work when the camera is connected later.</li><li>Direct V4L2 remains available for engineering comparisons and falls back safely when unsupported.</li><li>Manual exposure is optional, camera-dependent, and restored to automatic when tracking closes.</li></ul><p class=setup-help><a href=/help/camera>Open the Camera guide</a> for compatibility, exposure, lighting, and troubleshooting.</p><div class=controls><button type=submit id=camera-save>Save camera settings</button></div><p class=notice id=camera-notice role=status aria-live=polite></p></fieldset></section></form>
{{PLAYER_CONTENT}}
{{JOYSTICK_CONTENT}}
</section>
<section class=setup-phase id=setup-preferences aria-labelledby=setup-preferences-title>
<div class=setup-phase-heading><span class=setup-step>05 / PREFERENCES</span><h2 id=setup-preferences-title>Display and diagnostics</h2><p>Choose the idle Matrix display and what appears on your Dashboard.</p></div>
<section class=card style="margin-bottom:14px"><h2>Matrix attract mode</h2><form id=attract-form><label>Idle display<select id=matrix-attract><option value=on>On — full animation</option><option value=dim>Dim — gentle animation</option><option value=off>Off — connection pixels only</option></select></label><button type=submit>Save attract mode</button></form><p>Changes only the idle glove animation. Game displays, T, L, startup, errors, and pairing keep their normal brightness. Saves without restarting tracking.</p><details><summary>What the connection pixels mean</summary><p>In Off mode, four faint bottom-left pixels show: app running, console service reachable, authenticated console response, and a Wi-Fi or Ethernet link connected. The Networking pixel comes from the Controller’s physical network links, including Ethernet through a USB dock, independently of the console. It does not confirm an IP address or Internet access. These indicators do not prove that a game received input.</p></details><p id=attract-notice role=status></p></section>
{{STATISTICS_CONTENT}}
</section>
<section class=setup-phase id=setup-games aria-labelledby=setup-games-title>
<div class=setup-phase-heading><span class=setup-step>06 / GAME MAPPINGS</span><h2 id=setup-games-title>Choose games and profiles</h2><p>Advanced: review or edit which hand-control profile each NES game uses.</p></div>
{{GAMES_CONTENT}}
</section>
"""

_SETUP_LAYOUT_STYLE = """
.setup-jump{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:18px 0 30px}
.setup-jump a{display:flex;align-items:center;min-height:52px;padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:var(--panel);color:var(--ink);font:800 13px/1.3 system-ui;text-decoration:none}
.setup-jump a:hover,.setup-jump a:focus-visible{border-color:var(--cyan);background:#202b3a;outline:none}
.setup-phase{margin:0 0 32px;scroll-margin-top:20px}
.setup-phase-heading{margin:0 0 16px;padding:0 0 14px;border-bottom:1px solid var(--line)}
.setup-step{color:var(--cyan);font:800 12px/1.3 ui-monospace,monospace;letter-spacing:1.5px}
.setup-phase-heading h2{margin:6px 0;font:900 clamp(24px,4vw,31px)/1.1 system-ui}
.setup-phase-heading p{margin:0;color:var(--muted);max-width:760px}
.setup-phase>.card,.setup-phase>form>.card{margin-bottom:14px}
#setup-connect,#setup-preferences{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;align-items:start}
#setup-connect>.setup-phase-heading,#setup-preferences>.setup-phase-heading{grid-column:1/-1}
#setup-connect>.card,#setup-preferences>.card{margin:0!important}
#connection-status-card .connection-indicators{grid-template-columns:repeat(2,minmax(0,1fr))}
.setup-status-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px 16px;margin-top:14px}
.setup-status-summary p{margin:0;color:var(--muted);font-size:12px;overflow-wrap:anywhere}
.setup-status-summary strong{color:var(--ink)}
#setup-hand>#players{margin:0 0 14px}
#setup-hand>#joystick-settings{margin-top:0}
#setup-games #games-section{margin-top:0!important}
@media(max-width:850px){#setup-connect,#setup-preferences{grid-template-columns:1fr}#connection-status-card .connection-indicators{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:530px){.setup-jump{grid-template-columns:repeat(2,minmax(0,1fr))}.setup-status-summary{grid-template-columns:1fr}#connection-status-card .connection-indicators{grid-template-columns:1fr}}
"""
SETUP_CONTENT = SETUP_CONTENT.replace("</style>", _SETUP_LAYOUT_STYLE + "</style>", 1)
SETUP_CONTENT = SETUP_CONTENT.replace(
    "<h1>Setup</h1><p>Connect a supported game console, manage players, and choose how the camera and controls behave. For gesture practice and centring, open <a href=/learn>Glove Academy</a>. For a guided walkthrough, see <a href=/help/installation>Installation and setup</a>.</p>",
    "<h1>Connect. Centre. Play.</h1><p class=lead>Set up your console, controllers, camera and hand controls in order. Learn the gestures in <a href=/learn>Glove Academy</a>, or follow the <a href=/help/installation>Installation and setup guide</a>.</p>",
    1,
)
...
function renderPairing(){
 ...
 for(const id of ['platform','receiver','port','profile','connection-save'])$(id).disabled=lockConnection;
 ...
 $('pair-summary').textContent=pairStep>1?(platformNames[savedConfig?.platform]||'Platform not selected')+' · '+(savedConfig?.receiver||'Not configured')+' · '+methodLabel()+(pairStep>=3?' · Controller confirmation entered':''):'';
 ...
 const submitLabel=pairingBusy&&pairStep===3?'Pairing…':'Pair with console';
...
async function beginPairing(){
 ...
 try{
  const started=performance.now(),x=await api('/api/pair/begin',{host,platform:savedConfig.platform,method:chosen});
  if(!Number.isFinite(x.expires_in)||x.expires_in<=0||!x.certificate_id)throw Error('Confirmation is unavailable. Try again.');
  lockedUntil=started+x.expires_in*1000;prepared={host,platform:savedConfig.platform,method:chosen};retryConfirmation=false;
