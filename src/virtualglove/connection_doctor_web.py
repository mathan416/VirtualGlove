...
# File: src/virtualglove/connection_doctor_web.py
...
const signature=c=>JSON.stringify([String(c.platform||''),String(c.receiver||'').trim(),String(c.port??''),String(c.profile??''),c.connection_configured]);
function matches(c){return ['platform','receiver','port','profile'].every(k=>String($(k).value).trim()===String(c[k]??'').trim());}
...
for(const id of ['platform','receiver','port','profile'])$(id).addEventListener('input',invalidate);
...
$('doctor-run').onclick=async()=>{
 ...
 try{
  ...
  if(!matches(config)){put('saved','attention','The selected console or startup profile has unsaved changes. Select Save connection, then check again.');}
  else if(!config.receiver){put('saved','attention','Enter the console address above and select Save connection.');}
  else{
   ...
   try{
    ...
    put('receiver',active&&s.receiver_available===true?'pass':active&&s.receiver_available===false?'attention':'unknown',active&&s.receiver_available===true?'The running tracker reports an authenticated receiver handshake and packet sending. Game input delivery is not acknowledged.':active&&s.receiver_available===false?'The running tracker has no receiver link. Check the console receiver service, pairing, and UDP port.':'No active receiver-link check is available. When ready, use Dashboard to start controls with a game or manual profile, then check again.');
    ...
    if(s.worker_running===true&&s.game_session_active===true&&['super_glove_ball','bad_street_brawler',...Array.from({length:14},(_,i)=>'program_'+(i+1)),...'abcdefghi'.split('').map(x=>'program_'+x)].includes(profile)&&typeof core==='string'&&core&&core!=='unknown'&&['native','joystick'].includes(mode)){
     ...
     put('mapping',mode===expected?'pass':'attention',mode===expected?'The reported active game profile, emulator, and input mode agree. Installed core files and game-side input are not verified.':'The reported game profile and input mode disagree. Review Games and the console emulator selection, then restart the game.');
   ...
   put('device','unknown','The current protocol does not report virtual-input creation or native-core consumption. Verify input in the game on your console.');
