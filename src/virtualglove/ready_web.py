...
# File: src/virtualglove/ready_web.py
...
<h2 id=ready-title>Choose your player</h2><p id=ready-cue>Waiting for fresh Controller status and confirmation that controller output is safely stopped. This normally takes a few seconds.</p>
...
(()=>{
const $=id=>document.getElementById(id),session=(globalThis.crypto&&crypto.randomUUID)?crypto.randomUUID():`ready-${Date.now()}-${Math.random().toString(36).slice(2)}`,matcher=readyMatcher();
