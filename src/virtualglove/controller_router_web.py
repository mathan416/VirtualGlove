...
<p>Choose which configured controllers share each merged RetroArch player. Your original controllers keep working in the console menus. During Libretro gameplay, the merged players carry their assigned physical controllers; VirtualGlove joins its selected player in supported FCEUmm and stock Nestopia games.</p>
<p class=setup-status-note>Controllers configured in EmulationStation appear here automatically. Valid button remaps are picked up for the next game without changing the selected player.</p>
...
function sourceStatus(source){if(!source.connected)return'Unavailable';return source.mapping_status==='refreshed'?'Connected · Mapping refreshed':'Connected'}
function sourceLabel(source,id){const suffix=source?.identity_suffix||id.slice(-6),player=source?.assigned_player;return`${source?.name||'Configured controller'} · ${suffix}${player?` · Player ${player}`:''}`}
function render(data){snapshot=data;const body=byId('router-sources');body.replaceChildren();
 for(const source of data.inventory||[]){const row=document.createElement('tr'),name=document.createElement('td'),state=document.createElement('td'),assignment=document.createElement('td');name.textContent=`${source.name} · ${source.identity_suffix}`;state.textContent=sourceStatus(source);assignment.append(selectFor(source));row.append(name,state,assignment);body.append(row)}
...
byId('router-save').onclick=()=>work(byId('router-save'),async()=>{const players=[];for(let player=1;player<=4;player++){const sources=[...card.querySelectorAll('select[data-source]')].filter(item=>item.value===String(player)).map(item=>item.dataset.source);if(sources.length)players.push({player,sources})}const virtual=byId('router-virtual').value;render(await request('save',{revision:snapshot.revision,config:{players,virtualglove_player:virtual?Number(virtual):null}}));byId('router-notice').textContent='Controller Router assignments saved. They will apply to the next RetroArch launch.'});
byId('router-check').onclick=()=>work(byId('router-check'),async()=>{byId('router-notice').textContent='Press buttons or directions for the next 10 seconds…';const result=await request('check',{watch_ms:10000}),inventory=new Map((result.inventory||[]).map(item=>[item.id,item]));for(const select of card.querySelectorAll('select[data-source]'))select.closest('tr').children[1].textContent=sourceStatus(inventory.get(select.dataset.source)||{});const active=Object.entries(result.activity||{}).map(([id,controls])=>`${sourceLabel(inventory.get(id),id)}: ${controls.join(', ')}`),missing=(result.missing_sources||[]).map(id=>sourceLabel(inventory.get(id),id)),messages=[];if(active.length)messages.push(`Input received — ${active.join('; ')}.`);else if(!missing.length)messages.push('Controllers are connected; no input was pressed during the 10-second test.');if(missing.length)messages.push(`Unavailable — ${missing.join('; ')}. Reconnect ${missing.length===1?'this controller':'these controllers'} or change the assignment.`);byId('router-notice').textContent=messages.join(' ')});
