...
function matches(c){return ['platform','receiver','port','profile'].every(k=>String($(k).value).trim()===String(c[k]??'').trim())&&!$('rotate_token').checked;}
...
for(const id of ['platform','receiver','port','profile','rotate_token'])$(id).addEventListener('input',invalidate);
