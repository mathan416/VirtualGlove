...
for(const id of settingsFields.concat('rotate_token'))$(id).addEventListener('input',()=>{if(id==='camera_exposure')syncExposureFields();if(id==='platform')syncPlatformGuidance(true);if(pairStep===4){pairStep=1;clearSecrets()}renderPairing()});
...
pollCameraProfile();
const saveSettings=e=>{e.preventDefault();if(settingsBusy||pairingBusy||windowActive())return;if($('rotate_token').checked&&!confirm('Replace the pairing key and stop controller output? You must pair with the console again.'))return;const noticeId=e.submitter?.id==='camera-save'?'camera-notice':'notice';$(noticeId==='notice'?'camera-notice':'notice').textContent='';settingsBusy=true;action(e.submitter,noticeId,async()=>{
